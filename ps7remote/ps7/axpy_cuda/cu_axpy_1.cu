...
size_t find_10ms_size () {
  ...
  for (; size < 4UL * 1024 * 1024 * 1024; size *= 2) {
    ...
    cudaMallocManaged(&x, N * sizeof(float)); // Move array to GPU
    cudaMallocManaged(&y, N * sizeof(float));
    t0.start();
    checkCudaErrors(cudaDeviceSynchronize()); // check error.
    madd<<<1, 1>>>(N, 3.14159, x, y);         // Assign one block to sume up N length array. 
