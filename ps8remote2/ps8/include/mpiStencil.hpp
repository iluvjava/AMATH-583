    ...
    if (mysize != 1)
    {
        ...
        {   
            ...
            if (myrank == 0)
            {
                ...
                MPI::COMM_WORLD.Recv(&y(Height - 1, 1), Width, MPI::DOUBLE, 1, Sendup);
                MPI::COMM_WORLD.Send(&y(Height - 2, 1), Width, MPI::DOUBLE, 1, SendDown); 
            }   
            else
            {
                // Send and receive top row with: rank - 1
                MPI::COMM_WORLD.Send(&y(1, 1), Width, MPI::DOUBLE, myrank - 1, Sendup); 
                MPI::COMM_WORLD.Recv(&y(0, 1), Width, MPI::DOUBLE, myrank - 1, SendDown);
            }
        ...
        {   
            ...
            MPI::COMM_WORLD.Send(&y(1, 1), Width, MPI::DOUBLE, myrank - 1, Sendup);
            MPI::COMM_WORLD.Recv(&y(Height - 1, 1), Width,MPI::DOUBLE, myrank + 1, Sendup); 
            MPI::COMM_WORLD.Send(&y(Height - 2, 1), Width, MPI::DOUBLE, myrank + 1, SendDown);
            MPI::COMM_WORLD.Recv(&y(0, 1), Width, MPI::DOUBLE, myrank - 1, SendDown); 
        }
    }
}
