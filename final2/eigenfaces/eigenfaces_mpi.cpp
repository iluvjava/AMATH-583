...
auto scatter_images(const std::string& bin_file, size_t num_images, size_t height, size_t width) 
{
  ...
  // Distribute residual, make it even. 
  ...
  if (myrank == 0) 
  {
    ...
    if (mysize > 1)
    {
      ...
      for (size_t rank = 1; rank < mysize; ++rank) 
      {
        ...
        {
	        // !! Send z[count] Write me
          MPI::COMM_WORLD.Send(&z[count], height*width, MPI::DOUBLE, rank, 1);
          ++count;
        }
      }
    }
  ...
  else 
  {
    ...
    for (size_t i = 0; i < sendcounts[myrank]; ++i) 
    {
      ...
      MPI::COMM_WORLD.Recv(&myimg_data[i], height*width, MPI::DOUBLE, 0, 1);
      // !! Write me
    }
  }
  return myimg_data;
...
int main(int argc, char* argv[]) {
  ...
  MPI::COMM_WORLD.Allreduce(&local_C, &C, height*width, MPI::DOUBLE,MPI::SUM);
