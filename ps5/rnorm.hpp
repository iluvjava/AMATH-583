...
// Write me
double recursive_worker_a(const Vector& x,
                          size_t begin, 
                          size_t end,
                          size_t level)
{
  ...
  std::future<double> LeftSum = std::async(
    std::launch::async,
    recursive_worker_a, 
    std::cref(x), begin, Mid, level - 1
  );
  std::future<double> RightSum = std::async(
    std::launch::async,
    recursive_worker_a, 
    std::cref(x), Mid, end, level - 1
  );
