...
{
  // Forward shared memory for all threads in this block 
  // sdata has the same size as the bumber of threads in this block. 
  ...
  // Reduction across the grid using grid striding. 
  sdata[tid] = 0.0;
  ...
  __syncthreads();
   ...
   // Reduction across the block, dialating the summation node. 
  for (size_t s = 1; s < blockDim.x; s *= 2) 
  {
    size_t index = 2 * s * tid; // s=1-->2*tid, s=2-->s=4*tid, s=4-->8*tid
    if (index < blockDim.x) 
    {
