...
__global__ void madd(int n, float a, float* x, float* y) {
  ...
  // Each block all use the same mem. block defines the block of mem to work with. 
  // ThreadID defines the particular one to work on. 
...
size_t find_10ms_size () {
  ...
  for (; size < 4UL * 1024 * 1024 * 1024; size *= 2) {
    ...
    madd<<<1, 256>>>(N, 3.14159, x, y);       // 256 threads, 8 blocks of 32 threads. 
    checkCudaErrors(cudaDeviceSynchronize());
    t0.stop();
    cudaFree(x);
    cudaFree(y);
