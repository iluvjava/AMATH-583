...
public:
  ...
  void matvec(const Vector& x, Vector& y) const {
    for (size_t k = 0; k < storage_.size(); ++k) {
      y(std::get<0>(storage_[k])) += 
      std::get<2>(storage_[k]) * x(std::get<1>(storage_[k]));
    }
  }

  void t_matvec(const Vector& x, Vector& y) const {
    for (size_t k = 0; k < storage_.size(); ++k) {
      y(std::get<1>(storage_[k])) += 
      std::get<2>(storage_[k]) * x(std::get<0>(storage_[k]));
    }
  }
  ...
  void matmat(const Matrix& B, Matrix& C) const {
    for (size_t i = 0; i < storage_.size(); ++i) {
      for (size_t k = 0; k < B.num_cols(); ++k) {
        C(std::get<0>(storage_[i]), k) += 
        std::get<2>(storage_[i]) * B(std::get<1>(storage_[i]), k);
      }
    }
  }
